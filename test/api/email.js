...
User Length Preference: ${safeLength}

${safeLength === "breve" ? `
BREVE (Brief):
- Objetivo: Email conciso, directo al punto
- Longitud objetivo: 3-5 líneas en el cuerpo (sin contar saludo/despedida)
- Estructura: Saludo → Mensaje clave → Cierre
- Elimina información no esencial
- Usa frases cortas y directas
- Ideal para: confirmaciones, respuestas rápidas, recordatorios
` : ""}${safeLength === "detallado" ? `
DETALLADO (Detailed - Default):
- Objetivo: Email profesional estándar con contexto suficiente
- Longitud objetivo: 6-10 líneas en el cuerpo
- Estructura: Saludo → Contexto breve → Mensaje principal → Próximos pasos/Cierre
- Incluye contexto necesario sin excederse
- Balance entre claridad y profesionalismo
- Ideal para: la mayoría de comunicaciones profesionales
` : ""}${safeLength === "completo" ? `
COMPLETO (Complete):
- Objetivo: Email exhaustivo con todos los detalles relevantes
- Longitud objetivo: 10-15+ líneas en el cuerpo
- Estructura: Saludo → Contexto detallado → Múltiples puntos/secciones → Resumen/Próximos pasos → Cierre formal
- Incluye explicaciones, antecedentes, y detalles completos
- Puede usar bullets o numeración para organizar información
- Ideal para: propuestas, explicaciones complejas, comunicaciones formales importantes
` : ""}

CRITICAL: Respect the selected length while maintaining natural professional writing.
The email must feel complete for its purpose at the specified length.
`;
